...
<script type="text/javascript">
	...
	$(function() {
		...
		$.ajax({
			...
			// 用户菜单加载
			$.ajax({
				url : '${pageContext.request.contextPath}/json/self.json',
				type : 'POST',
				dataType : 'text',
				success : function(data) {
					var zNodes = eval("(" + data + ")");
		...
		$("#btnEp").click(function(){
			...
			// 将输入密码trim 
			if($.trim(txtNewPass) == ""){
				$.messager.alert("警告","密码不能为空，也不可以都为空白字符","warning");
				return;
			}
			...
			// 以Ajax方式进行密码修改
			$.post("${pageContext.request.contextPath}/user_editpwd.action", {"password": txtNewPass} , function(data){
				if(data == "success"){
					$.messager.alert("提示信息","修改密码成功","info");
				}else{
					$.messager.alert("错误信息","修改密码失败，请联系管理员","error");
				}
				// 隐藏修改密码窗体
				$("#editPwdWindow").window('close');
				$("#txtNewPass").val("");
	...
	function logoutFun() {
		// 弹出确认框
		$.messager.confirm("提示框","确认退出吗？", function(b){
			// b 就是用户选择内容，用户选择确定，b就是true，用户选择取消 b就是false
			if(b){
				// 退出
	...
	// 版权信息
	function showAbout(){
		$.messager.alert("ocj"," <br/> copy right reserved <br/>");
	}
</script>
</head>
<body class="easyui-layout">
	<div data-options="region:'north',border:false">
		<div>
			<img src="${pageContext.request.contextPath }/images/ocj.png"
				border="0">
		</div>
		<div id="sessionInfoDiv"
			style="position: absolute;right: 5px;top:10px;">
			[<strong>DEMO</strong>]，欢迎你！您使用[<strong>${pageContext.request.remoteAddr}</strong>]IP登录！
		</div>
				...
				<tr>
					<td style="width: 300px;">
						<div style="color: #999; font-size: 8pt;">
							 Powered by <a href="http://www.infosyschina.com">infosys</a>
						</div>
					</td>
